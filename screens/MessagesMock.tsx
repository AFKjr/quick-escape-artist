...
export default function MessagesMock({ navigation, route }: any) {
  ...
  useEffect(() => {
    const getMessageInfo = async () => {
      try {
        ...
        const messageId = route.params?.messageId || prefs.defaultMessageId;
        
        // Find the message template
        const messageTemplate = DEFAULT_MESSAGES.find(m => m.id === messageId) || DEFAULT_MESSAGES[0];
        if (messageTemplate) {
...
const styles = StyleSheet.create({
  wrap: { flex: 1, backgroundColor: '#fff', paddingTop: 60, paddingHorizontal: 16 },
  header: { 
    flexDirection: 'row', 
